...
const COGNITO_USER_POOL_ID = 'eu-west-2_1wHuDIFaq';
...
async function verifyToken(token: string, jwks: any): Promise<any> {
  ...
  // Verify the token
  return new Promise((resolve, reject) => {
    verify(token, pem, { algorithms: ['RS256'] }, (err: any, decoded: any) => {
      if (err) {
        reject(err);
      } else {
        resolve(decoded);
...
function redirectToLogin(request: any): any {
  ...
  const redirectUri = `https://${host}${request.uri}`;
  const loginUrl = `https://${cognitoDomain}.auth.${COGNITO_USER_POOL_REGION}.amazoncognito.com/login?response_type=token&client_id=YOUR_CLIENT_ID&redirect_uri=${encodeURIComponent(redirectUri)}`;
